...
if (isset($_GET['id_kamar'])) {
    ...
    $data = mysqli_query($konek,"SELECT * FROM kamar WHERE id_kamar='$id'");
    $data = mysqli_fetch_assoc($data);
}
    if (isset($_POST['update'])) {
        $id = $_GET['id_kamar'];
        $tipe = $_POST['tipe'];
        $faselitas = $_POST['faselitas'];
        $harga = $_POST['harga'];
        $status = $_POST['status'];

        mysqli_query($konek, "UPDATE kamar SET tipe='$tipe',faselitas='$faselitas',harga='$harga',status='$status' where id_kamar='$id'") or die(mysqli_error($konek));
  ...
  <path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3z"/>
	...
	<form class="sia-form my-5" action="" method="post" autocomplete="off" data-aos="zoom-in">
            <div class="form-header">
               <h3 class="text-capitalize open-sans text-sia-main fw-bold">form tambah kamar</h3>
               <p class="text-capitalize text-sia-grey open-sans">silahkan tambahkan kamar</p>
            </div>
            <div class="form-body">
               <div class="form-group">
                    <div class="my-4 d-flex sia-input">
                    <select name="tipe" id="" class="form-control" value="<?$data['tipe']?>">
                    <option value="regular">regular</option>
                     <option value="delux">delux</option>
                    </select>
                  </div>
                  <div class="my-4 d-flex sia-input">
                     <input type="text" class="form-control" placeholder="sialahkan isi faselitas" name="faselitas" value="<?$data['faselitas']?>"/>
                  </div>
                  <div class="my-4 d-flex sia-input">
                     <input type="text" class="form-control" placeholder="masukan harga" name="harga" value="<?$data['harga']?>"/>
                  </div>
                  <div class="my-4 d-flex sia-input">
                      <select name="status" id="" class="form-control" value="<?$data['status']?>">
                        <option value="ready">ready</option>
                         <option value="used">used</option>
                        </select>
                  </div>
                  <div class="btn-group d-flex">
                     <input type="submit" name="update" class="btn-main text-capitalize open-sans my-4" value="simpan"></input>
                  </div>
               </div>
            </div>
         </form>
